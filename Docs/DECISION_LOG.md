...
- **Question:** Are the World Creator product charter and architecture sufficiently coherent, bounded, lore-aligned, and implementation-ready to govern replacement of the prototype landscape generator?
- **Decision:** Approve the World Creator Charter and the hybrid causal world-compiler architecture. Use precision-safe deterministic world identity, a replaceable future coordinate seam, multi-scale causal plans, hybrid heightfield and bounded feature geometry, authored sample-to-rule grammars, purpose-conditioned playable geology, shared near/mid/far identity, stable persistence, and staged proof-driven migration. The Fractured Transect remains a non-canon proof fixture. Runtime implementation remains separately gated.
- **Why:** The audited prototype cannot reach the required canyon-system, site-integration, uniqueness, and composition quality through scalar-noise tuning alone. The approved documents preserve the dead Broken World constraints, future user-authored coordinate system, mid-range performance objective, exploration trust, BigARM traversal requirements, and rollback boundaries without claiming unproven runtime or visual results.
- **Evidence:** `Docs/WORLD_CREATOR_CHARTER.md`, `Docs/WORLD_CREATOR_ARCHITECTURE_PLAN.md`, the read-only generator audit contained in the architecture plan, the Crimson Desert research comparison, and the user's 2026-08-24 instruction delegating readiness approval to Gottspan.
- **Controlling files updated:** `Docs/WORLD_CREATOR_CHARTER.md`, `Docs/WORLD_CREATOR_ARCHITECTURE_PLAN.md`, `Docs/DOCS_INDEX.md`, and `Docs/DECISION_LOG.md`.
- **Consequences / follow-up:** The charter now governs World Creator product and quality decisions, while the architecture governs implementation guidance beneath it. The final thematic globe-coordinate design, regional map, first canon geology family, target hardware threshold, and topology-save policy remain reserved as documented. The next permissible action is a separately authorized Batch 0 readiness pass; approval does not authorize runtime code, Unity content changes, deletion of the current generator, cutover, package changes, publication, or release.
- **Supersedes:** The current shared geological surface implementation remains valid only as the prototype baseline and migration input; it no longer defines the long-term World Creator architecture.
