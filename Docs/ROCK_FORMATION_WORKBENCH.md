...
New standalone workbenches begin from the accepted Golden Rock calibration: a low, sideways `Blocky Monolith` with a raised shoulder and broad sloped face. Its resting rotation is baked into the editable source volumes, followed by a new local grounding pass. The root therefore remains at zero rotation and unit scale, and changing or rerolling the rock no longer requires rotating the Workbench or accepting accidental terrain burial.
...
The current individual-rock quality sequence is controlled by [ROCK_QUALITY_AND_PRODUCTION_PLAN.md](./ROCK_QUALITY_AND_PRODUCTION_PLAN.md). Stop after shaping one Golden Rock; do not roll its decisions across the catalog, formations, or terrain until the user accepts that reference.
