...
New standalone workbenches begin from the accepted Golden Rock calibration: a `1 m` wide by `0.75 m` long, low sideways `Blocky Monolith` with a raised shoulder, broad sloped face, and full edge damage. Creation assigns a fresh seed immediately. The resting rotation is baked into the editable source volumes, followed by a seed-stable random vertical-axis turn and a local grounding pass with four-to-twelve-percent burial variation. The root therefore remains at zero rotation and unit scale, while `Regenerate This Rock` preserves its facing/contact and `Generate New Rock` produces a new repeatable variation.
...
The current individual-rock quality sequence is controlled by [ROCK_QUALITY_AND_PRODUCTION_PLAN.md](./ROCK_QUALITY_AND_PRODUCTION_PLAN.md). The Golden Rock shape language is accepted; stop after the seeded facing/contact check before rolling it across the catalog, formations, or terrain.
