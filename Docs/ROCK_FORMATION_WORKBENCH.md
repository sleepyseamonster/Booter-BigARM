...
The main Rock Inspector intentionally shows only `Width`, `Height`, `Lopsidedness`, `Compaction`, and `Show Editing Volumes`. `Width` controls both horizontal axes while `Height` controls the vertical axis independently, so narrow pillars are not shortened by their footprint and broad rocks can remain genuinely low. The editable source-mass count is automatic: even the smallest generated rock now begins with five masses, while greater width or height adds masses for a richer silhouette. Click `Generate New Rock` for a new seed or `Update Current Rock With These Settings` to preserve the current seed. Mesh, collider, material, detailed surface, seed-gallery, and repair controls are under `Advanced`.

Generated rocks use three editable source shapes. `Weathered Block` supplies rounded mass, `Wedge` cuts a broad planar slope, and `Tapered Stone` supplies a truncated pyramidal profile. Those primitives are now assembled through five deterministic silhouette families: clustered `Boulder`, low `Slab`, faceted `Angular Chunk`, notched `Split Lobe`, and narrow `Shard`. The families change the dominant source shape, core-to-support proportions, growth direction, and overlap, so rerolls alter the actual exterior outline instead of merely changing surface noise on the same rounded cube. `Auto` selects a family from the rock seed and is intentionally hidden to keep the main workflow simple. Turn on `Show Editing Volumes`, expand the rock, and select an individual source volume to change its `Source Shape`, rotation, scale, or position manually.
