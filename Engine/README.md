...
The native application renders a perspective fixture and interactive inspector on Metal, with corrected geometry, a linear HDR/display pipeline, versioned inspection documents, and a [cookable texture catalog with GPU previews](./Docs/TEXTURE_PIPELINE_RESULT.md). See [the first-pass runtime foundation](./Docs/FIRST_PASS_RUNTIME.md). [Build and run it](./Docs/RUN_FOUNDATION.md), or review [the latest verified result and limitations](./Docs/OUTDOOR_GEOMETRY_RESULT.md).
