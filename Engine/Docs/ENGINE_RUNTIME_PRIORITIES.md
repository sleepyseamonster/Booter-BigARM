...
- **AI authoring interface:** follow the [human/AI build-out plan](./HUMAN_AI_ENGINE_BUILDOUT_PLAN.md): canonical scene documents, shared transform transactions, inspect/validate/apply/save/export/capture operations, isolated play sessions and runtime snapshots. Return operation identity, structured errors, result/version and completion status. Validate stale edits before commit and preserve undo/recovery. Begin with a local operation path and one real workflow; add a transport only when needed. Agent reasoning/network activity never blocks the real-time loop.
