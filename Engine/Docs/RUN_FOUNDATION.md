...
The window opens in borderless windowed fullscreen without requesting foreground activation. The normal scene viewer has no menus or control panels. For a scoped test, pass `--test-controls rock`, `engine`, `animation`, or `terrain` to show only that subsystem (with its required asset arguments). Click it to interact. Alt/Option + left-drag or right-drag outside the inspector orbits the inspection subject. Space + left-drag or middle-drag pans the view; the wheel changes distance; F recenters the pan. Start scene gestures outside the inspector. Pan position is stored with inspection settings, and Reset view clears it. These are scene controls; character mode retains Space to jump. Inspector controls change object rotation/color, light direction/intensity and camera settings. The turquoise column is a temporary 1.8-meter scale marker. F11 toggles borderless windowed fullscreen. Escape dismisses a menu or leaves fullscreen without quitting; use the window close control to exit.
...
From `Engine/`, launch the current build with the native recipe and existing cooked surfaces:
...
The current build now supports [live terrain/rock streaming in both apps](./STREAMING_FIRST_PASS_RESULT.md). Use the documented `--terrain`, `--stream-rock` and `--constraints` arguments; the workbench also accepts the existing material catalog. The dedicated rock-generator package remains the earlier milestone. World deltas and origin rebasing are still next-stage work.
