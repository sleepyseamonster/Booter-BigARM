...
Continue with **R5 correct materials, AO/depth and representative resource costs** after the completed R1–R4 correction batches. Visible gizmo and Game View presentation remain UI-owned acceptance gates. The typed play/durability services are complete; extending the R2 local scene socket with runtime verbs is a named integration follow-up rather than a reason to put disk I/O or lifecycle control in the frame loop. No new general audit or generator task is required.
