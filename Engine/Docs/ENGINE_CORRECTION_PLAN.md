...
Continue with **R6 native Windows validation and handoff** after the completed R1–R5 correction batches. Visible gizmo and Game View presentation remain UI-owned acceptance gates. The typed play/durability services and corrected material/occlusion/resource contracts are complete; extending the R2 local scene socket with runtime verbs is a named integration follow-up rather than a reason to put disk I/O or lifecycle control in the frame loop. No new general audit or generator task is required.
