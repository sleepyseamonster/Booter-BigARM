...
**Current build-out sequence (2026-09-14, audited):** [Human and AI engine build-out plan](./HUMAN_AI_ENGINE_BUILDOUT_PLAN.md) now defines the next architecture path: canonical scene document, shared transform transactions plus the first local AI adapter, runtime snapshot rendering, minimal Scene View gizmos, isolated Game View play sessions, expanded playtest commands and explicit authored-example-to-constraint capture. The immediate boundary is Phase 1 plus one atomic transform transaction; terrain, rocks and permanent world content remain downstream consumers.

**Latest human-AI authoring pass (2026-09-14):** [Renderer-independent scene document and local AI adapter](./HUMAN_AI_SCENE_DOCUMENT_RESULT.md) adds stable authored entities, deterministic hierarchy/world transforms, bounded versioned persistence, atomic transform transactions with rollback and undo/redo, and local `inspect_scene`/`inspect_entity`/`apply_transaction` commands. The adapter is UI/renderer-independent; all 15 configured core tests pass. Phase 3 render snapshots and the Scene View gizmo contract are next. Terrain, rocks, generators and permanent world content remain untouched.

**Latest occlusion pass (2026-09-14):** [Higher quality AO and contact shadows](./HIGH_QUALITY_AO_CONTACT_RESULT.md) replaces the four-tap display estimate with a 12-sample, edge-aware kernel and adds an opt-in depth/normal prepass with eight short sun-directed screen-space ray steps. Contact visibility affects direct sun only; AO remains a display-stage indirect-visibility approximation. Persisted controls migrate additively, Metal renderer captures pass, and all 14 native tests pass. This is engine infrastructure only; terrain, rocks, generators and Unity content are untouched.
