...
**Current build-out sequence (2026-09-14):** [Human and AI engine build-out plan](./HUMAN_AI_ENGINE_BUILDOUT_PLAN.md) now defines the next architecture path: canonical scene document, shared transform transactions, minimal Scene View gizmos, isolated Game View play sessions, local AI command operations, runtime snapshot rendering and explicit authored-example-to-constraint capture. The immediate boundary is Phase 1 plus one atomic transform transaction; terrain, rocks and permanent world content remain downstream consumers.
