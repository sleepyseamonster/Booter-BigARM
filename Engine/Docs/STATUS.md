...
**Latest user-directed work (2026-09-10):** the [wasteland terrain preview](./TERRAIN_PREVIEW_RESULT.md) connects terrain v2, three render LODs, original ground blending and seated v3 boulders to the existing streaming/save path. Open `out/wasteland-preview/Launch-Wasteland.command`. Native terrain/save checks and the packaged Metal load/retire/return run passed. This extends P18/P20; it does not complete origin shifting, imported heightmaps, terrain sculpting or native Windows validation.
