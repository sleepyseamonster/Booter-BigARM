...
#include <vector>
namespace engine {
struct TraversalProfile {std::string id;float radius=.35f,height=1.8f,maxSlope=45,maxStep=.35f;};
struct TraversalSample {float clearance=0,headroom=0,slope=0,step=0;};
struct Exclusion {std::string id;WorldPosition center;float radius=1;};
struct ReservedRoute {std::string id;WorldPosition start,end;float halfWidth=1;uint8_t agents=3;};
