...
RegionContent generate(const TerrainRecipe& terrain,const RockRecipe& recipe,const PlacementConstraints& constraints,Region region,const std::array<RockAsset,4>& rocks,const JobCancellation& token){
...
RegionStream::RegionStream(TerrainRecipe terrain,RockRecipe recipe,PlacementConstraints constraints,Attach attach,Detach detach)
    :terrain_(terrain),recipe_(recipe),constraints_(std::move(constraints)),rocks_(std::make_shared<std::array<RockAsset,4>>()),attach_(std::move(attach)),detach_(std::move(detach)),jobs_([](const auto& c){return c.bytes();}){
    validateTerrainRecipe(terrain_);validateRecipe(recipe_);validateConstraints(constraints_);if(!attach_||!detach_)throw std::invalid_argument("Region adapters required");
...
void RegionStream::update(const std::vector<StreamAnchor>& anchors){
    ...
    for(const auto& w:ordered){if(slots_.contains(key(w.region)))continue;
        if(slots_.size()>=25){auto old=std::find_if(slots_.begin(),slots_.end(),[](const auto& e){return !e.second.desired;});if(old==slots_.end())break;retire(old);}
        const auto region=w.region;const auto terrain=terrain_;const auto recipe=recipe_;const auto constraints=constraints_;const auto rocks=rocks_;
        if(epoch_==UINT64_MAX)throw std::overflow_error("Region epoch exhausted");
        auto ticket=jobs_.submit(owner(region),++epoch_,reservation_,[=](const auto& token){return generate(terrain,recipe,constraints,region,*rocks,token);});
        if(ticket)slots_.emplace(key(region),RegionSlot{region,*ticket,true,{},{},{}});
    }
    ...
    if(content->bytes()>64*1024*1024-residentBytes_){slot.error="Region CPU residency budget exhausted";return;}
    try{slot.ready=attach_(*content);slot.content=content;residentBytes_+=content->bytes();}
    catch(const std::exception& e){detach_(slot.region);slot.error=e.what();slot.ready={};}
}
