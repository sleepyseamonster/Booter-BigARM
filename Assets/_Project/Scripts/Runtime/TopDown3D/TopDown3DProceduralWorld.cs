...
namespace BooterBigArm.TopDown3D
{
    ...
    public sealed class TopDown3DProceduralWorld : MonoBehaviour
    {
        ...
        private void CreateProps(Transform chunkRoot, Vector2Int coordinate)
        {
            ...
            var footprints = new List<Vector3>(settings.PropsPerChunk);
            for (var i = 0; i < settings.PropsPerChunk; i++)
            {
                ...
                var footprintRadius = Mathf.Sqrt(width * width + depth * depth) * 0.5f;
                var edgeMargin = Mathf.Max(1.25f, footprintRadius + settings.PropSpacing);
                var availableSize = settings.ChunkSize - edgeMargin * 2f;
                if (availableSize <= 0f)
                {
                    continue;
                }
                ...
                for (var attempt = 0; attempt < settings.PropPlacementAttempts; attempt++)
                {
                    var localX = edgeMargin + (float)random.NextDouble() * availableSize;
                    var localZ = edgeMargin + (float)random.NextDouble() * availableSize;
                    ...
                    if (Vector2.Distance(worldPosition, spawnExclusionCenter)
                        < settings.ClearSpawnRadius + footprintRadius)
                    {
                        continue;
                    }

                    surfaceNormal = TopDown3DHeightSampler.SampleNormal(settings, worldX, worldZ);
                    if (Vector3.Angle(surfaceNormal, Vector3.up) > settings.MaximumPropSlope
                        || OverlapsFootprint(footprints, worldPosition, footprintRadius))
                    {
                        continue;
                    }
                    ...
                    localPosition = new Vector3(
                        ...
                        localZ);
                    footprints.Add(new Vector3(worldX, worldZ, footprintRadius));
                    placed = true;
                    break;
                }
                ...
                var yaw = (float)random.NextDouble() * 180f;
                prop.transform.localRotation =
                    Quaternion.FromToRotation(Vector3.up, surfaceNormal) * Quaternion.AngleAxis(yaw, Vector3.up);
        ...
        private bool OverlapsFootprint(List<Vector3> footprints, Vector2 position, float radius)
        {
            for (var i = 0; i < footprints.Count; i++)
            {
                var existing = footprints[i];
                var minimumDistance = radius + existing.z + settings.PropSpacing;
                if ((position - new Vector2(existing.x, existing.y)).sqrMagnitude < minimumDistance * minimumDistance)
                {
                    return true;
                }
            }
