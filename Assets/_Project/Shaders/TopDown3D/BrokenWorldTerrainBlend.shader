    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 TerrainFragment(Varyings input) : SV_Target
            {
                ...
                // Vertex colour is generated by TopDown3DWorldGenerator in world space:
                // R deposited sand, G gravel/talus, B exposed bedrock, A weathering/lithology.
                // The shader only softens those geological weights; it does not invent a
                // competing material distribution from unrelated patch noise.
                float sandSignal = saturate(input.geologyWeights.r);
                float gravelSignal = saturate(input.geologyWeights.g);
                float rockySignal = saturate(input.geologyWeights.b);
                float weathering = saturate(input.geologyWeights.a);
